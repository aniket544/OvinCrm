...
import { toast } from 'react-hot-toast';
import * as XLSX from 'xlsx';
import './LeadManager.css'; // ✅ CSS File Import ki hai
...
const FollowUpModal = ({ lead, onClose, onConfirm }) => {
    ...
    return (
        <div className="modal-content">
            <h3 className="modal-header">📞 Follow-up: {lead.company}</h3>
            
            <div>
                <label>Next Follow-up Date</label>
                <input type="date" className="lead-input" value={followUpDate} onChange={(e) => setFollowUpDate(e.target.value)} />
            </div>
            
            <div>
                <label>Task Priority</label>
                <select className="lead-select" value={priority} onChange={(e) => setPriority(e.target.value)}>
            ...
            <div>
                <label>Remarks</label>
                <textarea rows="2" className="lead-input" value={remarks} onChange={(e) => setRemarks(e.target.value)} />
            </div>

            <div className="modal-actions">
                <button onClick={onClose} className="btn btn-cancel">Cancel</button>
                <button onClick={handleConfirm} className="btn btn-primary">Create Task</button>
            </div>
        </div>
    );
};

// --- 2. PAYMENT DETAILS MODAL COMPONENT ---
const PaymentDetailsModal = ({ lead, onClose, onConfirm }) => {
    const [paymentData, setPaymentData] = useState({ so_no: '', amount: '', advance: '', remaining: '0.00', invoice: '', remark: '' });
    ...
    return (
        <div className="modal-content">
            <h3 className="modal-header">💰 Finalize Deal: {lead.company}</h3>
            
            <div className="form-row">
                <div className="form-col">
                    <label>Sales Order No.</label>
                    <input type="text" name="so_no" className="lead-input" value={paymentData.so_no} onChange={handleInputChange} placeholder="SO-XXX" />
                </div>
                <div className="form-col">
                    <label>Invoice No.</label>
                    <input type="text" name="invoice" className="lead-input" value={paymentData.invoice} onChange={handleInputChange} placeholder="INV-XXX" />
                </div>
            </div>
            ...
            <div className="form-row">
                <div className="form-col">
                    ...
                    <input type="number" name="amount" className="lead-input" value={paymentData.amount} onChange={handleInputChange} placeholder="0.00" />
                </div>
                <div className="form-col">
                    <label>Advance Paid</label>
                    <input type="number" name="advance" className="lead-input" value={paymentData.advance} onChange={handleInputChange} placeholder="0.00" />
                </div>
            </div>
            ...
            <div className="form-row">
                <div className="form-col">
                    ...
                    <input type="text" readOnly className="lead-input" value={`₹ ${paymentData.remaining}`} style={{color: '#ffbb33', fontWeight: 'bold'}} />
                </div>
            </div>
            ...
            <div className="form-row">
                <div className="form-col">
                    <label>Remarks</label>
                    <textarea name="remark" className="lead-input" value={paymentData.remark} onChange={handleInputChange} rows="2" />
                </div>
            </div>

            <div className="modal-actions">
                <button onClick={onClose} className="btn btn-cancel">Cancel</button>
                <button onClick={handleConfirm} className="btn btn-primary">Record Payment</button>
            </div>
        </div>
    );
};
...
const LeadManager = () => {
    ...
    const [newLead, setNewLead] = useState({
        date: getCurrentDateTime(), sno: '', company: '', name: '', contact: '', email: '', 
        address: '', note: '', purpose: '', status: 'New'
    });
    ...
    const handleFileChange = (e) => {
        ...
        reader.onload = async (evt) => {
            ...
            if (data.length === 0) { toast.error("File khali hai!"); return; }
            ...
            for (const row of data) {
                ...
                try { 
                    ...
                    console.error("Row fail:", row, err);
                }
            }
            toast.success(`${count} Leads Imported!`, { id: toastId });
            e.target.value = null;
        };
        reader.readAsArrayBuffer(file);
    };
    ...
    const confirmMoveToSales = async (lead, followUpDate, priority, remarks) => {
        ...
        const taskPayload = {
            ...
            task_type: 'Call', next_follow_up: followUpDate, priority: priority,
            remarks: remarks, date: getCurrentDateTime().split('T')[0],
        };
    ...
    const confirmPaymentRecord = async (leadId, paymentData) => {
        ...
        try {
            ...
            await axios.post(`${LEAD_API_URL}${leadId}/convert/`, paymentData, headers); 
            setLeads(leads.map(lead => lead.id === leadId ? { ...lead, status: 'Converted' } : lead));
            toast.success("Deal Closed! 💰", { id: toastId });
            setShowPaymentModal(false);
        } catch (error) { 
            toast.error("Error finalizing deal.", { id: toastId }); 
        }
    };
    ...
    const handleDeleteTrigger = (id) => {
        toast((t) => (
            <div style={{ color: '#fff' }}>
                <div style={{ fontSize: '14px', fontWeight: 'bold', marginBottom: '10px' }}>Delete Lead?</div>
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                    <button onClick={() => { confirmDelete(id); toast.dismiss(t.id); }} className="btn btn-danger">Yes</button>
                    <button onClick={() => toast.dismiss(t.id)} className="btn btn-cancel">No</button>
                </div>
            </div>
        ...
        <>
            <div className="lead-container">
                <div className="lead-header">
                    <div className="lead-title">Lead Manager</div>
                    <div className="action-buttons">
                        <button className="btn btn-primary" onClick={handleSave}>+ Save</button>
                        <input type="file" accept=".xlsx, .xls" ref={fileInputRef} style={{display: 'none'}} onChange={handleFileChange} />
                        <button className="btn btn-import" onClick={handleImportClick}>📥 Import</button>
                ...
                <div className="table-responsive">
                    <table className="lead-table">
                        <thead>
                            <tr>
                                <th>Date & Time</th><th>S.No</th><th>Company</th><th>Name</th><th>Contact</th>
                                <th>Email</th><th>Address</th><th>Note</th><th>Purpose</th><th>Status</th>
                                <th className="text-center">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr style={{ background: '#2a2a2a' }}>
                                <td><input type="datetime-local" name="date" value={newLead.date} onChange={handleInputChange} className="lead-input" /></td>
                                <td><input type="text" name="sno" value={newLead.sno} onChange={handleInputChange} placeholder="1" className="lead-input" style={{width: '50px'}} /></td>
                                <td><input type="text" name="company" value={newLead.company} onChange={handleInputChange} placeholder="Company" className="lead-input" /></td>
                                <td><input type="text" name="name" value={newLead.name} onChange={handleInputChange} placeholder="Name" className="lead-input" /></td>
                                <td><input type="text" name="contact" value={newLead.contact} onChange={handleInputChange} placeholder="Phone" className="lead-input" /></td>
                                <td><input type="text" name="email" value={newLead.email} onChange={handleInputChange} placeholder="Email" className="lead-input" /></td>
                                <td><input type="text" name="address" value={newLead.address} onChange={handleInputChange} placeholder="Address" className="lead-input" /></td>
                                <td><input type="text" name="note" value={newLead.note} onChange={handleInputChange} placeholder="Note" className="lead-input" /></td>
                                <td>
                                    <select name="purpose" value={newLead.purpose} onChange={handleInputChange} className="lead-select">
                                        <option value="">Select</option>
                                        {purposeOptions.map((o, i) => <option key={i} value={o}>{o}</option>)}
                                    </select>
                                </td>
                                <td>
                                    <select name="status" value={newLead.status} onChange={handleInputChange} className="lead-select">
                                        <option>New</option><option>Contacted</option><option>Interested</option><option>Closed</option>
                                    </select>
                                </td>
                                <td>✨</td>
                            ...
                            {leads.map((l, index) => (
                                <tr key={l.id} className="hover-row">
                                    <td style={{fontSize: '12px', color: '#bbb'}}>{formatDateTime(l.date)}</td>
                                    <td>{l.sno || index + 1}</td>
                                    <td style={{color: '#fff', fontWeight: 'bold'}}>{l.company}</td>
                                    <td>{l.name}</td>
                                    <td style={{color: '#00ffcc'}}>{l.contact}</td>
                                    <td>{l.email}</td>
                                    <td>{l.address}</td>
                                    <td>{l.note}</td>
                                    <td>{l.purpose}</td>
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: '5px', justifyContent: 'center' }}>
                                            <button className="btn btn-outline-follow" onClick={() => handleMoveToSalesTrigger(l)} title="Follow Up">📞</button>
                                            {l.status !== 'Converted' && (
                                                <button className="btn btn-outline-follow" onClick={() => handleConvertTrigger(l)} title="Convert">💰</button>
                                            )}
                                            <button className="btn btn-outline-delete" onClick={() => handleDeleteTrigger(l.id)}>🗑️</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
